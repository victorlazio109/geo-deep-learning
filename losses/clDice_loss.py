...
class clDiceLoss(nn.Module):
    ...
    def __init__(self, smooth=1.0, eps=1e-7, ignore_index=None, weight=None):
        ...
        self.dl = DiceLoss(weight=self.weight)

    def forward(self, output, target, target_skeleton, output_seg):
        ...
        DL = self.ce(output_seg, target)
        CE = self.ce(output, target)
        bs = target.size(0)
        num_classes = output_seg.size(1)
        dims = (0, 2)

        cl_output = soft_skeletonize(output_seg)
        output_seg = output_seg.log_softmax(dim=1).exp()
        cl_output = cl_output.log_softmax(dim=1).exp()

        output_seg = output_seg.view(bs, num_classes, -1)
        ...
        tflat = norm_intersection(target_skeleton, output_seg.type_as(target_skeleton),
                                  smooth=self.smooth, eps=self.eps, dims=dims)
